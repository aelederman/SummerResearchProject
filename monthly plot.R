...
monthly_data %>%
  mutate(sector = recode(str_sub(cell_id, 1, 1), !!!sector_map)) %>%
  group_by(sector) %>%
  summarise(num_firms = n_distinct(cell_id), .groups = "drop") %>%
  ggplot(aes(x = reorder(sector, num_firms), y = num_firms)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  coord_flip() +
  labs(
    title = "Number of Firms by Industry",
    x     = "Industry",
    y     = "Number of Firms"
  ) +
  theme_minimal()