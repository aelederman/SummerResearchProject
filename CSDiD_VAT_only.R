...
# 0. Read the CODIMP codes to keep (impuesto.csv)
#    This file should have columns "CODIGO" and "NOMBRE".
...
# 2a. Filter to keep only the CODIMP codes that are affected by e-invoicing
raw_data <- raw_data %>%
...
# 2) Impute October 2023 (202310) for every small‐firm row:
monthly_data_imputed <- monthly_data %>%
  mutate(
    Y = if_else(
...
final_for_did <- monthly_data_imputed %>%
  select(
    id,                      # unique id for each cell
    period,                  # 1,2,3,... for each month
    first_treated_period,    # first treated period index
